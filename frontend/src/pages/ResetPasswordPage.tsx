    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 px-6 relative">
        <div className="w-full max-w-[440px] glass-panel rounded-3xl p-8 text-center shadow-2xl">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">Request Processed</h3>
          <p className="text-slate-300 text-sm leading-relaxed mb-6">{successMsg}</p>
          <Link
            to="/login"
          ...
          {token ? (
            ...
            <form onSubmit={handleSubmit(onSubmitReset)} className="space-y-5">
              <div>
                ...
                {errors.newPassword && (
                  <p className="mt-1 text-xs text-red-400 font-semibold">{errors.newPassword.message as string}</p>
                )}
              </div>
